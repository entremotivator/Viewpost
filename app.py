...
class UltimateSocialMediaManager:
    ...
    def create_media_folder(self, drive_service):
        """Create or get Social Media Assets folder in Google Drive"""
        try:
            # Check if folder already exists
            folder_name = "Social Media Assets"
            ...
            if folders:
                ...
                st.session_state.media_folder_id = folder_id
                return folder_id
            else:
                # Create new folder
                folder_metadata = {
            ...
            st.error(f"Error creating media folder: {str(e)}")
            return None
    
    def upload_file_to_drive(self, drive_service, file_content, filename, mime_type):
        """Upload file to Google Drive and return public URL"""
        try:
            # Get or create media folder
            folder_id = self.create_media_folder(drive_service)
            if not folder_id:
                return None, "Failed to create media folder"
    ...
    def get_existing_media_from_sheets(self, df):
        """Extract all existing media URLs from the sheets"""
        media_items = []
        ...
        for idx, row in df.iterrows():
            ...
            if image_url:
                media_items.append({
                    ...
                    'url': image_url,
                    'post_index': idx,
                    'message_preview': self.safe_str_conversion(row.get('Message', ''))[:50] + "..."
                })
...
def show_manual_post_creation(manager, df, client):
    """Manual post creation form with real-time analysis and media upload"""
    with st.form("manual_post_form", clear_on_submit=True):
        ...
        with col1:
            ...
            # Media Upload and Selection Section
            st.subheader("🎬 Media Content")
            
            media_tab1, media_tab2, media_tab3 = st.tabs(["📤 Upload New", "📁 Pick Existing", "🔗 Add URL"])
            ...
            with media_tab1:
                ...
                uploaded_file = st.file_uploader(
                    ...
                    help="Upload images (JPG, PNG, GIF) or videos (MP4, MOV, AVI, MKV, WEBM)"
                )
                ...
                if uploaded_file:
                    ...
                    if file_type.startswith('image/'):
                        st.image(uploaded_file, caption=f"Preview: {file_name}", use_column_width=True)
                        st.session_state.uploaded_image_content = file_content
                        st.session_state.uploaded_image_name = file_name
                        st.session_state.uploaded_image_type = file_type
                    elif file_type.startswith('video/'):
                        st.video(uploaded_file)
                        st.session_state.uploaded_video_content = file_content
                        st.session_state.uploaded_video_name = file_name
                        st.session_state.uploaded_video_type = file_type
                    
                    if st.button("📤 Upload to Google Drive", type="primary"):
                        drive_service, error = manager.setup_google_drive()
                        if error:
                            st.error(f"❌ {error}")
                        else:
                            with st.spinner("Uploading to Google Drive..."):
                                ...
                                public_url, upload_error = manager.upload_file_to_drive(
                                    drive_service, file_content, unique_filename, file_type
                                )
                                ...
                                if upload_error:
                                    ...
                                    if file_type.startswith('image/'):
                                        st.session_state.new_image_url = public_url
                                    else:
                                        st.session_state.new_video_url = public_url
            
            with media_tab2:
                st.markdown("**Pick from Existing Media:**")
                
                # Get existing media from sheets
                existing_media = manager.get_existing_media_from_sheets(df)
                
                if existing_media:
                    # Filter by type
                    media_type_filter = st.selectbox("Filter by type:", ["All", "Images", "Videos"])
                    
                    filtered_media = existing_media
                    if media_type_filter == "Images":
                        filtered_media = [m for m in existing_media if m['type'] == 'image']
                    elif media_type_filter == "Videos":
                        filtered_media = [m for m in existing_media if m['type'] == 'video']
                    
                    if filtered_media:
                        # Display media grid
                        cols = st.columns(3)
                        for idx, media in enumerate(filtered_media):
                            with cols[idx % 3]:
                                st.markdown(f"**{media['type'].title()}**")
                                ...
                                try:
                                    ...
                                    st.markdown(f"🔗 [Preview]({media['url']})")
                                
                                st.markdown(f"*From: {media['message_preview']}*")
                                
                                if st.button(f"Use This {media['type'].title()}", key=f"use_media_{idx}"):
                                    if media['type'] == 'image':
                                        st.session_state.selected_image_url = media['url']
                                        st.success(f"✅ Image selected!")
                                    else:
                                        st.session_state.selected_video_url = media['url']
                                        st.success(f"✅ Video selected!")
                    else:
                        st.info("No media found matching the filter.")
                else:
                    st.info("No existing media found in your posts. Upload some media first!")
            
            with media_tab3:
                st.markdown("**Add Media URLs Manually:**")
                manual_image_url = st.text_input("🖼️ Image URL", placeholder="https://example.com/image.jpg")
                manual_video_url = st.text_input("🎥 Video URL", placeholder="https://example.com/video.mp4")
                
                if manual_image_url:
                    try:
                        st.image(manual_image_url, caption="Image Preview", use_column_width=True)
                        st.session_state.manual_image_url = manual_image_url
                    except:
                ...
                if manual_video_url:
                    try:
                        ...
                        st.session_state.manual_video_url = manual_video_url
                    except:
                        st.warning("⚠️ Could not preview video")
            ...
            with col1_2:
                # Show selected media URLs
                final_image_url = ""
                final_video_url = ""
                
                if 'new_image_url' in st.session_state:
                    final_image_url = st.session_state.new_image_url
                    st.success(f"✅ Uploaded image ready")
                elif 'selected_image_url' in st.session_state:
                    final_image_url = st.session_state.selected_image_url
                    st.success(f"✅ Selected image ready")
                elif 'manual_image_url' in st.session_state:
                    final_image_url = st.session_state.manual_image_url
                    st.success(f"✅ Manual image URL ready")
                
                if 'new_video_url' in st.session_state:
                    final_video_url = st.session_state.new_video_url
                    st.success(f"✅ Uploaded video ready")
                elif 'selected_video_url' in st.session_state:
                    final_video_url = st.session_state.selected_video_url
                    st.success(f"✅ Selected video ready")
                elif 'manual_video_url' in st.session_state:
                    final_video_url = st.session_state.manual_video_url
                    st.success(f"✅ Manual video URL ready")
        ...
        if submitted and message:
            ...
            if success:
                ...
                # Clear session state media URLs
                for key in ['new_image_url', 'selected_image_url', 'manual_image_url', 
                           'new_video_url', 'selected_video_url', 'manual_video_url']:
...
def main():
    ...
    with st.sidebar:
        ...
        # Sheet URL - Hardcoded for convenience
        if 'sheet_url' not in st.session_state:
            st.session_state.sheet_url = "https://docs.google.com/spreadsheets/d/1NFVvhSeE5YrjRB4Q4vA5Ar7ohPFnQgzjkKdOGx1G0Wo/edit?usp=drivesdk"
        
        sheet_url = st.text_input(
            "Google Sheets URL",
            value=st.session_state.sheet_url,
            help="Your Google Sheets URL (pre-filled)"
        )
